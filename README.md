...
*Percentual total (linhas): 95.26%* (acima do mínimo exigido de 85%).
...
Relatório HTML: cobertura/coverage/index.html (abrir no navegador).
...
- *O que é auditado*: ações sensíveis de usuário e admin — LOGIN_SUCCESS,
  LOGIN_FAILED, LOGOUT, USER_REGISTER, BOOK_CREATE, BOOK_UPDATE,
  BOOK_DELETE, AUTHOR_CREATE, CATEGORY_CREATE, ORDER_CREATE,
  RECOMMENDATION_REQUEST.
- *Onde fica armazenado*: tabela AuditLog no PostgreSQL (Prisma).
  Campos principais: id, userId, action, entity, entityId,
  metadata (JSON), ipAddress, createdAt.
- *Como foi implementado*: service dedicado (logAudit) chamado a partir
  de cada rota de API sensível. Centraliza a escrita do registro, captura
  IP do request e serializa metadata em JSON.
- *Classes/arquivos participantes*:
  - src/lib/audit.ts — função logAudit (service central).
  - prisma/schema.prisma — modelo AuditLog + enum AuditAction.
  - src/auth.ts — eventos LOGIN_SUCCESS, LOGIN_FAILED.
  - src/app/api/logout/route.ts — LOGOUT.
  - src/app/api/register/route.ts — USER_REGISTER.
  - src/app/api/books/route.ts e src/app/api/books/[id]/route.ts —
    BOOK_CREATE, BOOK_UPDATE, BOOK_DELETE.
  - src/app/api/authors/route.ts — AUTHOR_CREATE.
  - src/app/api/categories/route.ts — CATEGORY_CREATE.
  - src/app/api/orders/route.ts — ORDER_CREATE.
  - src/app/api/recommendations/route.ts — RECOMMENDATION_REQUEST.
  - src/app/admin/audit-logs/page.tsx + src/app/api/admin/audit-logs/route.ts
    — painel de visualização com filtro por ação.

## Integração com Serviço Externo

- *Serviço externo: **OpenAI API* (modelo gpt-4o-mini).
- *Para que é usado*: geração de recomendações personalizadas de e-books
  para o usuário, a partir do histórico de compras, categorias favoritas e
  interesses declarados. A resposta da IA é restrita aos livros existentes
  no catálogo (evita "alucinação").
- *Classes/arquivos participantes*:
  - src/lib/openai.ts — cliente OpenAI + montagem do prompt + parsing
    da resposta JSON.
  - src/app/api/recommendations/route.ts — rota que invoca o serviço,
    persiste em Recommendation e grava o log de auditoria
    (RECOMMENDATION_REQUEST).
  - src/app/recommendations/page.tsx — UI que consome a rota.
- *Configuração (variáveis de ambiente, sem segredos)*:
  - OPENAI_API_KEY — chave da API OpenAI (definida no .env, ver
